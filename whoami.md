---
layout: page
title: whoami  
permalink: /whoami.html
---

# <span class="term-prompt">&gt;</span> gayathri@defendwithgr <span class="term-path">~</span> cat <span class="term-filename">whoami.md</span> <span class="term-cursor">█</span>

## Who I Am

I've always believed career twists lead to the right path. After years in SAP techno-functional consulting, I found my passion in cybersecurity and did my post-graduate certificate in cybersecurity operations, then started my journey as a **SOC Analyst at Kryptos Technologies**—thriving on threat monitoring and incident response (Best Employee of the Month award 🏆).

## What I Do

### Current Role
**Teaching Assistant - Cybersecurity** | York University  
*Jan 2025 - Present*

Supporting course delivery for Cybersecurity Operations, facilitating discussions, evaluating student participation, and providing feedback. Collaborating with students on capstone projects while fostering engagement and ensuring a positive learning experience.

### Professional Experience

**SOC Analyst** | Kryptos Technologies | *Apr 2024 - Aug 2025*

- Enhanced network security, reducing non-compliant device incidents by 25% through policy configuration and Azure AD integration
- Improved SIEM alert handling SOPs, achieving 30% reduction in response times and 20% boost in efficiency
- Managed security incident triage in 24/7 SOC environment, maintaining SLA adherence
- Analyzed over 100 suspicious emails monthly to identify IOCs and malicious behavior
- Conducted real-time triage and root cause analysis of phishing, malware, and unauthorized access incidents

**Associate Consultant - SAP Security** | Capgemini | *June 2017 - June 2023*

- Managed user provisioning for 1000+ employees with proper access control
- Conducted security reviews for SAP migrations and version upgrades (5000+ users)
- Optimized role structures for diverse business processes
- Worked on SAP cross functional modules like SD, MM, FI and ABAP.
- Helped with Retrofitting and development of new roles and authorization objects.
- Mentored junior team members in access management principles

## Certifications

```bash
$ ls certifications/
```

- ⏳ **Microsoft SC-500** - Identity and Access Administrator (In Progress)
- ✅ **Microsoft SC-200** - Security Operations Analyst (Aug 2024)
- ✅ **CompTIA Security+** SY0-601 (May 2024)
- 📜 **SOC2** - LinkedIn (Nov 2024)
- 📜 **Fortinet NSE 1,2,3** - Network Security Associate (Aug 2023)
- ✅ **ISC² CC** - Certified in Cybersecurity (Jun 2023)
- 📜 **Qualys** - Vulnerability Management Foundation (Jun 2023)
- 📜 **Splunk** - Fundamentals (Mar 2023)
- 📜 **NIST** - Risk Management Framework (Sep 2022)

## Technical Arsenal

```yaml
SIEM Tools:
  - Microsoft Defender, Intune, Azure Security Center
  - Splunk, IBM QRadar

Security Tools:
  - Burp Suite, Metasploit, Nessus, Nmap, OpenVAS, Wireshark

Cloud Platforms:
  - Microsoft Azure, AWS, Google Cloud Platform
  - Azure AD (Entra ID), SSO, Access Control

Frameworks:
  - NIST SP 800-61 Rev 2, NIST CSF, RMF
  - ISO 27001, CIS Controls, GDPR, OWASP Top 10, SOC 2

Scripting & CLI:
  - PowerShell, Bash, Python
  - Linux/Windows CLI (grep, tail, netstat, ipconfig)

Networking:
  - TCP/IP, VPN, Firewalls, DNS/DHCP
  - Azure Networking

Infrastructure:
  - Windows Server (2016/2019/2022)
  - Active Directory, Group Policy Objects (GPO)
  - SAP Security
```

## Community Involvement

I'm passionate about giving back to the cybersecurity community:

- 🤝 **ISC2 Toronto Chapter** - Director of Professional Development & Volunteer
- 👩‍💻 **Leading Cyber Ladies** - Community Member
- 🎯 **CTF Player** - Continuous hands-on practice
- 📖 **Ilaiya Thalaimurai, India** - Program Director, designing crash courses and mentoring students

## Beyond Cybersecurity

When I'm not in front of a terminal, you'll find me:

- 📚 Reading — always have a book on the go
- 🎵 Listening to music
- 🏠 Working on home cybersecurity projects — homelabs, CTF challenges, and tinkering

## Areas of Interest

Focus Areas: **Threat Detection & Response** | **Security Monitoring** | **Incident Investigation** | **AI in Cybersecurity**

---

### Let's Connect

**LinkedIn**: [linkedin.com/in/gayathri-rajamohan](https://www.linkedin.com/in/gayathri-rajamohan/)  
**GitHub**: [github.com/defendwithgr](https://github.com/defendwithgr)  
**Email**: gayathrirajamohan@gmail.com

**Languages**: English, Tamil, French (Beginner)

---

*"Beyond my cybersecurity work, I enjoy staying active and engaged through various hobbies and interests."*
